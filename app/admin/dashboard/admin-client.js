...
export default function AdminDashboardClient({ initialQuestions }) {
  ...
  const handleEditQuestion = async (originalSerial, data) => {
    try {
      ...
      setQuestions((prev) =>
        prev.map((q) => (q.serial === originalSerial ? updatedQuestion : q))
      );
      setEditingItem(null);
      setFormData({
        serial: "",
  ...
  const getNextSerialNumber = () => {
    ...
    const maxSerial = Math.max(
      ...questions.map((q) => parseInt(q.serial) || 0)
    );
    return maxSerial + 1;
  };
          ...
          >
            ...
            >
              <div>
                ...
                />
                {errors.serial && (
                  <p className="text-red-400 text-sm mt-1">{errors.serial}</p>
                )}
              </div>
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex gap-2">
                          ...
                          <button
                            onClick={() =>
                              setConfirmModal({ show: true, item: question })
                            }
