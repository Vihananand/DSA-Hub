...
const detectPlatform = (questionLink) => {
  ...
  if (url.includes("geeksforgeeks.org") || url.includes("gfg.org"))
    return "gfg";
  if (
    url.includes("codingninjas.com") ||
    url.includes("codingninjas.in") ||
...
export function QuestionCard({
  ...
  const getDifficultyColor = (difficulty) => {
    switch (difficulty?.toLowerCase()) {
      ...
      case "medium":
        return "from-amber-500/20 to-orange-600/20 border-amber-500/30 text-amber-300";
      case "hard":
  ...
  return (
    <div className="group" style={{ transform: "translateZ(0)", zIndex: 1 }}>
      <Card className="p-6 h-full">
        ...
        <div className="flex items-start justify-between mb-4">
          ...
          <div
            className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider border bg-gradient-to-r ${getDifficultyColor(
              question.difficulty
            )} shrink-0`}
          >
            ...
            <label className="flex items-center gap-2 text-sm text-gray-300 cursor-pointer hover:text-white transition-colors">
              <div className="relative">
                <input
                  ...
                  onChange={() =>
                    onProgressChange?.(question.serial, "revised")
                  }
                  className="sr-only"
                />
                <div
